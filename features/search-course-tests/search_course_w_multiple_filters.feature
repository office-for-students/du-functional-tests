Feature: Course Search engine with multiple filters combination test
As a user
I should be able to search university Course with Study Mode and Placement filter and get results

Scenario Outline: Course search with multiple filters combination does not return error
    ...
    When  I select Placement filter with text "With placement year"
    And   I pause for <wait_time>ms
    Then  I expect that element "#placement-yes" is selected
